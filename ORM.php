...
class ORM
{
    ...
    public static function config()
    {
        ...
        if ($argNum == 1) {
            ...
            self::_config(array($key => $value));
        }
    }

    private static function _config($arr)
    {
    ...
    {
        $orm = new self($table);
        if (self::$_config['long_id_name']) {
            $orm->_primaryKey = $table.'_id';
        } else {
            $orm->_primaryKey = 'id';
        }
        return $orm;
    ...
    public function update($arr)
    {
        ...
        foreach ($arr as $key => $value) {
            ...
            $exprs[] = self::_backQuoteWord($key)."=$value";
        }
    ...
    {
        return new DataWrapper($this->_table, array());
    }
...
class DataWrapper
{
    private $_info;
    private $_table;
    private $_dirts = array();
    
    public function __construct($table, $info, $id=null)
    {
        $this->_table = $table;
        $this->_info  = $info;
        $this->_id    = $id;
    ...
    public function set($a)
    {
        ...
        if ($argNum == 1) {
            ...
            $this->_set(array($a, func_get_arg(1)));
        }
    }
    ...
    {
        return array_key_exists($name, $this->_dirts);
    }

    public function dirty($name)
    {
        if (array_key_exists($name, $this->_info) && !$this->isDirty($name)) {
            $this->_dirts[$name] = $this->_info[$name];
        }
    }

    public function save()
    {
        $orm = ORM::forTable($this->_table)->whereId($this->_id);
        if ($this->id) {
            $orm->update($this->_dirts);
        } else {
            $this->_id = $orm->insert($this->_dirts);
            $this->_info[$orm->primaryKey()] = $this->_id;
        }
    }

    public function delete()
    {
        ORM::forTable($this->_table)->whereId($this->_id)->delete();
    }
}
